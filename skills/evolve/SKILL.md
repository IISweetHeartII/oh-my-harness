...
<!-- Modified from revfactory/harness (Apache-2.0, Copyright 2025 robin):
     added the opt-in evidence-driven autonomous mode pointer
     (upstream PR #45, @epoko77-ai). -->
...
위 Phase 1~5 는 **피드백 주도**다 — 사람이 말해야 시작된다. 결정적 검증기와 분할 가능한
반복 과제 집합을 **둘 다** 갖춘 하네스라면, 실행 트레이스에서 실패를 채굴해 스스로
바운디드 편집을 제안하고 held-in/held-out 비퇴행 게이트로만 채택하는 자율 루프를 추가로
가동할 수 있다.

자격 분류(A/B/C), 실패 시그니처 `(c, q, m)`, 수용 규칙, 되돌리기·로깅 프로토콜은
`../harness/references/self-evolution-loop.md` 참조.

**자격 게이트를 통과하지 못한 하네스에는 적용하지 마라.** 주관적 신호에 숫자를 붙이면
없는 게이트보다 나쁘다 — 틀렸을 때 더 믿게 된다.
